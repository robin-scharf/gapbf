...
class DatabaseStoreMixin:
    """Run and attempt CRUD plus config fingerprinting for RunDatabase."""

    _lock: Lock
    connection: sqlite3.Connection

    # Public API kept as a method so callers can use ``database.attempt_hash_for(...)``.
    attempt_hash_for = staticmethod(attempt_hash_for)
    ...
    def get_terminal_attempt_entry(
        ...
        attempt_hash = attempt_hash_for(device_id, config.grid_size, attempt)
        with self._lock:
            row = self.connection.execute(
                """
                SELECT attempt, attempt_hash, result_classification
