...
class Config(BaseModel):
    ...
    def validate_path_constraints(self) -> "Config":
        ...
        for field_name in ("path_prefix", "path_suffix", "excluded_nodes", "test_path"):
            ...
            if invalid_nodes:
                raise ValueError(
                    f"{field_name} contains invalid nodes for a "
                    f"{self.grid_size}x{self.grid_size} grid: {invalid_nodes}"
                )
        ...
        if merged_constraint is None:
            raise ValueError(
                "path_prefix and path_suffix cannot describe a valid non-repeating path"
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Config(grid={self.grid_size}x{self.grid_size}, "
            f"len={self.path_min_length}-{self.path_max_length})"
        )
